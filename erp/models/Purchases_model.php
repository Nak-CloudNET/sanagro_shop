...
class Purchases_model extends CI_Model
{
    ...
    {
        $this->db->select('companies.address,phone')
            ...
            foreach ($items as $item) {
                ...
                */
                if($item['type'] == 'service'){
					...
					if($item['net_shipping'] > 0){
						$item['net_shipping'] = $item['net_shipping'];
					}else{
						$item['net_shipping'] = 0;
					}
				}
	...
	public function getPaymentByExpenseID($id)
    {
        $q = $this->db->get_where('payments', array('expense_id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function getCurrentBalance($pur_id)
	{
