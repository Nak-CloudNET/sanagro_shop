...
class Pos_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id,products.cost, products.code, products.name, products.type,categories.type AS cate_type,warehouses_products.product_id, warehouses_products.quantity, price, tax_rate, tax_method,products.image, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
    ...
    {
        $this->db->select('sales.*, users.username,erp_tax_rates.name AS tax,erp_payments.paid_by,erp_payments.cheque_no,erp_payments.cc_no,erp_payments.cc_type');
    ...
    */
	...
	public function addSeperate($items,$tab,$id){
		if($items && $tab){
				$dataBill = $this->getsuspendedBill_itemsByID($id);
				//$this->erp->print_arrays($dataBill);
				if($dataBill->id){
					unset($dataBill->id);
					$this->db->update("erp_suspended",array("status"=>"1"),array("id"=>$tab));
					...
					if($dd){
						...
						$this->db->update("erp_suspended_bills",array("total"=>($dd->total+$data_sum->gtotal)),array("id"=>$dd->id));
						$dataBill2 = $this->getsuspendedBill_itemsByID($id);
						$this->db->update("erp_suspended_bills",array("total"=>($dataBill2->total - $data_sum->gtotal)),array("id"=>$id));
					}else{
						$this->db->insert("erp_suspended_bills",$dataBill);
						 $billid = $this->db->insert_id();
						 $sub = 0;
						...
						$this->db->update("erp_suspended_bills",array("total"=>$data_sum->gtotal,'suspend_id'=>$tab),array("id"=>$billid));
						$dataBill2 = $this->getsuspendedBill_itemsByID($id);
						$this->db->update("erp_suspended_bills",array("total"=>($dataBill2->total - $data_sum->gtotal)),array("id"=>$id));
					}
				}
