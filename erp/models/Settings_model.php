...
class Settings_model extends CI_Model
{
	...
	public function Old_Customer_Group($id=null)
	{
		...
		$this->db->select("categories.id,categories_group.percent,categories.name")
    ...
    {
        $this->db->select('date, bom.name, bom_items.quantity, bom_items.cost, noted, created_by, status, product_name, product_code, product_variants.name as var_name');
		$this->db->from('bom');
		$this->db->join('bom_items', 'bom_items.bom_id = bom.id');
