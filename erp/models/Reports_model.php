...
*/
	...
	public function getSupplierByID($id){
		$this->db
                ...
                ->where(array('purchases.status' => 'received', 'purchases.payment_status <>' => 'paid', 'companies.id' => $id))
	...
	*/
	...
	public function getQtyByWare($pid,$wid,$product2,$category2,$start,$end){
		...
		if($start && $end){
			$this->db->where('purchase_items.date >="'. $this->erp->fsd($start).'" AND purchase_items.date<="'. $this->erp->fsd($end).'"');
		}
		$q = $this->db->get("products");
		if ($q->num_rows() > 0) {
            return $q->row();
	...
	public function getCatExpenseReport($reference_no,$from_date,$to_date,$per_page,$ob_set){
		$this->db->select("gl_trans.narrative,expenses.account_code")
			...
			if($from_date && $to_date){
				$this->db->where('expenses.date >="'.$this->erp->fsd($from_date).' 00.00" AND expenses.date<="'.$this->erp->fsd($to_date).' 23.59"');
			}
