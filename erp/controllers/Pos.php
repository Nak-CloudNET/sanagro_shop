...
class Pos extends MY_Controller
{
    ...
    function index($sid = null, $sale_order_id=null, $combine_table = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			if($this->input->post('other_cur_paid')){
				$other_cur_paid = $this->input->post('other_cur_paid');
			}
			
			
			$other_cur_paid = array_filter($other_cur_paid);
			if(!empty($other_cur_paid)){
            ...
            if (!$suspend) {
				...
				if(isset($p_cur) && empty($_POST['amount'][0])){
					...
					$pp[] = $amount;
					if($_POST['paid_by'][$j] != 'deposit' && $_POST['paid_by'][$j] != 'depreciation') {
						$this->site->updateReference('sp');
					}
                }
	...
	function close_register_popup($register_id){
		...
		$q = $this->db->get();
		if($q->num_rows()>0){
			$this->close_register($q->row()->user_id,$q->row()->date);
		}
		return false;
	}
    function close_register($user_id = NULL,$rdate = NULL)
    {
        ...
        if ($row) {
            ...
            if ($opt->price != 0) {
				if($customer_group->makeup_cost == 1){
					...
					$row->cost  = $row->cost + (($row->cost * $percent->percent) / 100);
				}else{
					$row->price = $opt->price + (($opt->price * $customer_group->percent) / 100);
				}
            } else {
			...
			//$this->erp->print_arrays($row->cost);
            ...
            if ($row->tax_rate) {
                ...
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'item_price' => $row->price,'orderqty'=>isset($orderqty->quantity), 'group_prices' => $group_prices, 'all_group_prices' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent);
            } else {
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'item_price' => $row->price,'orderqty'=>$orderqty->quantity, 'group_prices' => $group_prices, 'all_group_prices' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent);
            } 
    ...
    function view_cabon_bk($sale_id = NULL, $modal = NULL){
        $this->erp->checkPermissions('index');
        if ($this->input->get('id')) {
            $sale_id = $this->input->get('id');
        }
        ...
        $this->data['modal'] = $modal;
        $this->data['page_title'] = $this->lang->line("invoice");
        $this->load->view($this->theme . 'pos/bk_invoice',$this->data);
    }
	...
	function add_seperate(){
		...
		//$this->erp->print_arrays($items,$tab);
		if($items && $tab){
			if($this->pos_model->addSeperate($items,$tab,$id)){
