...
class Drivers extends MY_Controller
{
    ...
    {
        $this->erp->checkPermissions('index');
		if($this->Owner || $this->Admin || $this->GP['drivers-edit']){
			$edit_driver = "<a class=\"tip\" title='" . lang("edit_driver") . "' href='" . site_url('drivers/edit/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a>";
		}else{
			$edit_driver = "";
		}
		if($this->Owner || $this->Admin || $this->GP['drivers-delete']){
			$delete_driver = " <a href='#' class='tip po' title='<b>" . lang("delete_driver") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('drivers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
		}else{
			$delete_driver = "";
		}
		$this->load->library('datatables');
        $this->datatables
            ...
            ->add_column("Actions", "<div class=\"text-center\"> ".$edit_driver.' '.$delete_driver."</div>", "id");
        //->unset_column('id');
        echo $this->datatables->generate();
    }
    ...
    function view($id = NULL)
    {
        ...
        $this->load->view($this->theme.'drivers/view',$this->data);
    }
    ...
    {
        $this->erp->checkPermissions('edit', NULL, 'drivers');
		$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
		$this->data['id']=$id;
		$this->data['driver']=$this->companies_model->getCompanyByID($id);
		$this->data['modal_js'] = $this->site->modal_js();
	...
	function create_driver(){
		...
		$this->erp->checkPermissions('add', NULL, 'drivers');
	...
	function delete($id=null){
		...
		$this->erp->checkPermissions('delete', NULL, 'drivers');
