...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var pos_head2 = '</h4><h5>Room | Table: <span style="font-size:18px;">'+ $("#suspend_name").val() +'</span><br/>Username: '+u_username+'<br/>Customer: '+$('#select2-chosen-1').text()+'<br>In: '+formattedDate+'<br/></h5>';
		...
		$.each(positems, function (i, e) {
			...
			if (item_type == 'standard' && item.options !== false) {
				...
				if(item_type_by!=""){
					...
					{
					var oprTr = '<tr class="row_' + item_id + ' order_print_food " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + ' (' + item_code + ') '+ (item_note? '['+item_note+']':"") +'</td><td>' + formatDecimal(item_qty) +'</td></tr>';
					
					   $("#order-table-food").append(oprTr);
					   
					}else if(item_type_by=='drink' && item_print==0){
						
					var oprTr = '<tr class="row_' + item_id + ' order_print_drink " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + ' (' + item_code + ') '+ (item_note? '['+item_note+']':"") +'</td><td>' + formatDecimal(item_qty) +'</td></tr>';
