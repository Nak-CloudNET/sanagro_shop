...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
	 ...
	 * Edit Row Method
	 ...
	 $(document).on('click', '#editItem', function () {
		...
		var pgroup_price = $('#pgroup_price').val();
		...
		slitems[item_id].row.real_unit_price = pgroup_price == 0 || pgroup_price == undefined ? price : price / opt_cur,
		...
		if(item.all_group_price !== false) {
			$.each(item.all_group_price, function () {
				...
				if(opt ==0){				
					...
					price = (default_price + ((default_price * (item.customer_percent == undefined ? 0 : item.customer_percent))/100));
					...
					if(this.price_group_id == opt && this.price != 0 && this.price != '' && this.price != null) {
						...
						{
							rate  = (this.price)-0;
							price = (rate+((rate * (item.customer_percent == undefined ? 0 : item.customer_percent))/100));
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
				...
				if(site.settings.attributes == 0){
					...
					if(item.row.item_load==0)
					{
						...
						if(group_prices){
							...
							item.row.price_id = group_prices[0].price_group_id;
							group_price_id    = item.row.group_price_id; 
						}
				    }
			...
			if (ds.indexOf("%") !== -1) {
				var pds = ds.split("%");
				if (!isNaN(pds[0])) {
					item_discount = parseFloat(((unit_price*item_qty) * parseFloat(pds[0] / 100)));
				} else {
					item_discount = parseFloat(ds / item_qty);
				}
			} else {
			...
			if(delivery_id){
				tr_html += '<td><input style="pointer-events:none;" class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
			}else{
			...
			if (item_promotion == 1 && (current_date >= start_date && current_date <= end_date)){
				...
				total += (item_tax_method == 0?(((parseFloat(real_unit_price) * parseFloat(item_qty))) - (item_discount)):(((parseFloat(unit_price) * parseFloat(item_qty))) - (item_discount) + parseFloat(pr_tax_val)* parseFloat(item_qty)));
			}
			
			count += parseFloat(item_qty);
			an++;
			if(item_type == 'standard' && parseFloat(item_qty) > parseFloat(item_qoh)) {
