    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            if (owner || admin) {
				...
				/* Sub Total */
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + (item_tax_method==0?formatMoney((parseFloat(formatPurDecimal(net_unit_cost))* parseFloat(item_qty) - (parseFloat(item_discount)*parseFloat(item_qty)) ) ):formatMoney( (parseFloat(net_unit_cost)* parseFloat(item_qty)+ (parseFloat(pr_tax_val)* parseFloat(item_qty)  ) - (parseFloat(item_discount)*parseFloat(item_qty)) ) )) + '</span></td>';
			} else {
					if (site.settings.product_discount == 1) {
	                //tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatPurDecimal(0 - (item_discount * item_qty)) + '</span></td>';
						tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><input type="text" class="form-control text-center rdiscount text-danger" value="' + item_ds + '" id="sdiscount_' + row_no + '"></td>';
			...
			/* Total */
			total +=  (item_tax_method==0?( (parseFloat(formatPurDecimal(net_unit_cost))* parseFloat(item_qty) - (parseFloat(item_discount)*parseFloat(item_qty)) ) ):( (parseFloat(net_unit_cost)* parseFloat(item_qty)+ (parseFloat(pr_tax_val)* parseFloat(item_qty)  ) - (parseFloat(item_discount)*parseFloat(item_qty)) ) ));
