                ...
                <ul class="nav navbar-nav pull-right">
					...
					<li class="dropdown">
                        <a class="btn borange pos-tip" id="close_register" title="<span><?=lang('close_register')?></span>" data-placement="bottom" data-html="true" data-backdrop="static" href="<?=site_url('pos/close_register')?>" data-toggle="modal" data-target="#myModal">
                            <i class="fa fa-money"></i><p><?= lang('clear') ?></p>
                        </a>
                    </li>
                    <li class="dropdown">
                    ...
                    <?php if ($Owner || $Admin) {?>
                        ...
                        <li class="dropdown hidden-xs">
                            <a class="btn bblue pos-tip" title="<?=lang('list_open_registers')?>" data-placement="bottom" href="<?=site_url('pos/registers')?>">
                                <i class="fa fa-list"></i><p><?= lang('registers') ?></p>
                            </a>
                        </li>
                        <li class="dropdown hidden-xs">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" id="pr_popover_content">
                <table class="table table-bordered table-striped table-condensed table-hover"
					...
					<tr>
                        <td><?= $pos_settings->discount ?></td>
                        <td><?= lang('discount') ?></td>
						<td>
							<button type="button" class="btn bdarkGreen col-md-12 addButton" id="discount_shortcut"> <?= $pos_settings->discount ?></button>
						</td>
                    </tr>
                    </tbody>
                </table>
    ...
    $(document).ready(function () {
		...
		*/
		...
		$("#Pcode").autocomplete({
			...
			if(item.no_pro == 0){
				...
				var inner_html  = 	'<td style="width:50px;height:30px;">' +
								...
								//	item.strap +
								//'</td>' +
								'<td class=""><a href="#ShowImage" data-toggle="modal"><img class="passimage" src="<?= $assets ?>assets/uploads/thumbs/'+item.pic+'" alt="'+item.pic+'" style="width: 70px; height: 70px; margin: 0 auto; display: block;"></a></td>';
			}
			return $( "<tr></tr>")
		...
		$('#myModal').bind('keypress', function(e){
			if (e.keyCode == 13) {
                e.preventDefault();
               $("input[name*='add_customer']").trigger('click');
            }
		});
		
		$(document).on("click", ".passimage", function () {
			var img = $(this).attr('alt');
			var image = '<img src="<?= $assets ?>assets/uploads/'+img+'" style="width:100%;"/>';
			$('.getImg').html(image);
		});
		...
		$("#Pstrap").autocomplete({
			...
			var inner_html  = 	'<td style="width:50px;height:30px;">' +
								...
								'<td style="width:64px;">'+
									item.strap +
								'</td>'+
								'<td class=""><a href="#ShowImage" data-toggle="modal"><img class="img-circle passimage" src="<?= $assets ?>assets/uploads/thumbs/'+item.pic+'" alt="'+item.pic+'" style="width:30px; height:30px;"></a></td>';
			return $( "<tr></tr>")
				.data( "item.autocomplete", item )
				.append(inner_html)
				.appendTo($('.test'));
