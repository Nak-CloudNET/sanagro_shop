					...
					<?php
						...
						if($this->uri->segment(7)){
							...
							foreach($dataAsset->result() as $row){
								...
								foreach($assetDetails->result() as $asD){
									...
									?>
									<td class="text-right"><div class="fix-text"><?= $b_amt?></div></td>
									<?php
									echo '</tr>';
								...
								if($total_amt >0){
									$total_amt = number_format(abs($total_amt), 2);
								}else{
									$total_amt = '( ' . number_format(abs($total_amt), 2) . ' )';
								}
								...
								echo '<td><div class="fix-text text-right">' . $total_amt . '</div></td>';
								echo '</tr>';
							}
						?>
							<tr>
							...
							foreach($dataLiability->result() as $rowlia){
								...
								foreach($libDetails->result() as $libD){
									...
									$lib_amt_dis_in = '';
									if((-1)*$libD->amount < 0){
										$lib_amt_dis_in = '( ' . number_format(abs($libD->amount), 2) . ' )';
									}else{
										$lib_amt_dis_in = number_format(abs($libD->amount), 2);
									}
									?>
									...
									?>
									<td><div class="fix-text text-right"><?= $lib_amt_dis_in ?></div></td>
									<?php
									echo '</tr>';
								...
								*/
								...
								echo '<td colspan="2"><div class="fix-text text-right">' . number_format(abs($total_amt), 2) . '</div></td>';
								echo '</tr>';
							}
						...
						?>
								...
								for($c= 0; $c < count($new_billers); $c++){
									...
									*/
									...
									if($total_lib_amt < 0){
										...
										echo '<td><div class="fix-text text-right"><b>'. number_format($total_lib_amt,2) .'</b></div></td>';
									}
							...
							foreach($dataEquity->result() as $rowequity){
								...
								foreach($eqDetails->result() as $eqD){
									...
									<?php foreach ($new_billers as $bi2) {
										...
										if($bi2_biller == $eqD->biller_id){
											...
											$eq_display = 0;
											if((-1)*$eqD->amount < 0){
												$eq_display = '( ' . number_format(abs($eqD->amount), 2) . ' )';
											}else{
												$eq_display = number_format(abs($eqD->amount), 2);
											}
											...
											echo '<td><div class="fix-text text-right"></div></td>';
										}
									}
									$eq_display1 = 0;
									if((-1)*$eqD->amount < 0){
										$eq_display1 = '( ' . number_format(abs($eqD->amount), 2) . ' )';
									}else{
										$eq_display1 = number_format(abs($eqD->amount), 2);
									}
									?>
									<td><div class="fix-text text-right"><?= $eq_display1 ?></div></td>
									<?php
									echo '</tr>';
								...
								$sum_eq_per_acc = array();
								echo '<td colspan="2"><div class="fix-text text-right">' . number_format(abs($total_amt_eq), 2) . '</div></td>';
								echo '</tr>';
							...
							if($total_eq_sum <0){
								$total_eq_sum = '( ' . number_format(abs($total_equity-$total_retained),2) . ' )';
							}else{
								$total_eq_sum = number_format(abs($total_equity-$total_retained),2);
							}
						?>							
							...
							if($total_eq_sum <0){
								$total_eq_sum = '( ' . number_format(abs($total_eq_sum), 2) . ' )';
							}else{
								$total_eq_sum = number_format(abs($total_eq_sum),2);
							}
							?>
