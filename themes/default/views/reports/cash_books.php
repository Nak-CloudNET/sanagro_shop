                        ...
                        <tbody>
							<?php
								...
								foreach($acc as $val){
									...
									$gl_tranStart->where(array('tran_date < '=> ($start_date2." 00.00"), 'account_code'=> $val->accountcode));
									$startAmount = $gl_tranStart->get()->row();
									
									$endAccountBalance = 0;
									$getListGLTran = $this->db->select("*")->from('gl_trans')->where('account_code =', $val->accountcode);
								
									$getListGLTran->where('tran_date >="'.$start_date2.' 00.00" AND tran_date<="'.$end_date2.' 23.59"');
